...
void TileParser::parseStyleLayers(util::ptr<StyleLayerGroup> group, float z) {
    if (!group) {
        return;
    }
    ...
    for (const util::ptr<StyleLayer> &layer_desc : group->layers) {
        ...
        if (layer_desc->bucket) {
            ...
            if (bucket_it == tile.buckets.end()) {
                ...
                std::unique_ptr<Bucket> bucket = createBucket(layer_desc->bucket, z);
                if (bucket) {
...
std::unique_ptr<Bucket> TileParser::createBucket(util::ptr<StyleBucket> bucket_desc, float z) {
    if (!bucket_desc) {
    ...
    if (layer_it != vector_data.layers.end()) {
        ...
        if (bucket_desc->type == StyleLayerType::Fill) {
            applyLayoutProperties<FillProperties>(*bucket_desc, z);
            ...
            applyLayoutProperties<LineProperties>(*bucket_desc, z);
            ...
            applyLayoutProperties<SymbolProperties>(*bucket_desc, z);
